...
static void _configureDacTimer(void) {
  ...
  TIM7->PSC = 31; //32MHz/(31+1) for 1MHz input gives 15Hz minimum output
...
void configure_timers() { _configureDacTimer(); }
...
void configure_dac() {
  ...
  DAC->DHR12R1 = 2000;
  DAC->CR |= DAC_CR_BOFF1 | DAC_CR_EN1; //Enable output buffer, enable DAC
}
